      ...
      perSystem =
        ...
        in
        {
          ...
          packages = rec {
            ...
            axon-static = pkgs.pkgsStatic.callPackage ./devshell/package-static.nix {
              inherit name version;
              rustPlatform = rustPlatformMusl;
            };
            completions = pkgs.runCommand "axon-completions" { } ''
              mkdir -p $out/share/{bash-completion/completions,fish/vendor_completions.d,zsh/site-functions}
              ${axon}/bin/axon completions bash  > $out/share/bash-completion/completions/axon
              ${axon}/bin/axon completions fish  > $out/share/fish/vendor_completions.d/axon.fish
              ${axon}/bin/axon completions zsh   > $out/share/zsh/site-functions/_axon
            '';
            check-format = pkgs.callPackage ./devshell/format.nix { };
            deb-x86_64 = pkgs.callPackage ./devshell/package-nfpm.nix {
              inherit name version;
              axon-static = if isCrossFromAarch64 then static-x86_64 else axon-static;
              packager = "deb";
              arch = "amd64";
            };
